...
export interface TutorialVideosTab {
  id: string;
  name: string;
  description?: string;
  active?: boolean;
...
export interface TutorialVideosNavbarHeaderProps {
  tabs?: TutorialVideosTab[];
  onTabChange?: (tabId: string) => void;
  defaultActiveTab?: string;
  className?: string;
}

const TutorialVideosNavbarHeader: React.FC<TutorialVideosNavbarHeaderProps> = ({
  tabs = [
  ...
  const handleTabChange = (tabId: string) => {
    setActiveTab(tabId);
    onTabChange?.(tabId);
    console.log(`Active tutorial videos tab changed to: ${tabId}`);
  };

  return (
    <header className={`relative overflow-hidden ${className}`}>
      {/* Tutorial Videos Navigation Tabs */}
          ...
          >
            {tabs.map((tab, index) => (
              ...
              >
                ...
                {activeTab === tab.id && (
                  <motion.div
                    layoutId="activeTutorialVideosTabIndicator"
...
export default TutorialVideosNavbarHeader;