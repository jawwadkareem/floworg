...
const ContactNavbar: React.FC<ContactNavbarProps> = ({
  tabs = [
  ...
  const handleTabChange = (tabId: string) => {
    setActiveTab(tabId)
    onTabChange?.(tabId)
    console.log(`Active contact tab changed to: ${tabId}`)
  }
            ...
            <button
              ...
              className={`relative px-6 py-3 font-medium text-sm transition-all duration-300 rounded-full border ${
                activeTab === tab.id
                  ...
                  : "bg-white text-gray-600 border-gray-300 hover:border-teal-400 hover:text-gray-700"
              }`}
            >
              <motion.span
                initial={{ opacity: 0 }}
